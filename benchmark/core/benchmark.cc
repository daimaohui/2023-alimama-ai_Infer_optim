...
int main(int argc, char* argv[]) {
  ...
  for (auto bench_model_conf : *benchmark_config->mutable_bench_model_config()) {
    ...
    auto sample_file = bench_model_conf.mutable_sample_file();
    if (!FileExists(conf_dir, sample_file)) {
      LOG(ERROR) << "Invalid runmeta path: " << *sample_file;
      ShowHelp(argv[0]);
      return 1;
    }
